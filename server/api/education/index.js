...
router.get('/users/:uid', auth.hasScope('education:read'), controller.get);

router.put('/:id/users/me', auth.isAuthenticated(), controller.updateMe);
router.put(
  '/:id/users/:uid',
  auth.hasScope('education:write'),
  controller.update
);
...
router.post('/users/:uid', auth.hasScope('education:write'), controller.create);
